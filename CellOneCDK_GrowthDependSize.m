...
classdef CellOneCDK_GrowthDependSize
    properties
        g
        pCDK 
        Size
    ...
    methods
        function obj = CellOneCDK_GrowthDependSize(g, pCDK, Size, CDK, Threshold_D, CycleLength, Phase, BirthSize, DaughterSize1, DaughterSize2)
            if(nargin > 0)
                obj.g = g;
                obj.pCDK = pCDK;
                obj.Size = Size;
            ...
            for j = 1:Steps
                ...
                for i = 1:n
                  if tissue(i).Phase < 2;
                     m = m + 1;
                     ActiveCells(m) = i;
                     tissue(i).Size = tissue(i).Size + (tissue(i).Size * (-0.0002 * tissue(i).Size + 0.0338));  
                    ...
                    elseif tissue(i).Phase == 1
                        ...
                        tissue(i).Phase = tissue(i).Phase + 1;
                        tissue(length(tissue) + 1) = CellOneCDK_GrowthDependSize(RGRsDaughter1(i), pCDK, tissue(i).DaughterSize1, 0, Threshold_D, 0, 0, tissue(i).DaughterSize1, 0, 0); 
                        tissue(length(tissue) + 1) = CellOneCDK_GrowthDependSize(RGRsDaughter2(i), pCDK, tissue(i).DaughterSize2, 0, Threshold_D, 0, 0, tissue(i).DaughterSize2, 0, 0);
