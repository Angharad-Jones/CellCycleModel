% Class CellOneCDK_GrowthDependSize


classdef CellOneCDK_GrowthDependBirthSize
    properties
        RGRvar
        pCDK 
        Size
    ...
    methods
        function obj = CellOneCDK_GrowthDependBirthSize(RGRvar, pCDK, Size, CDK, Threshold_D, CycleLength, Phase, BirthSize, DaughterSize1, DaughterSize2)
            if(nargin > 0)
                obj.RGRvar = RGRvar;
                obj.pCDK = pCDK;
                obj.Size = Size;
        ...
        function tissue = GrowthIteration(tissue, Steps, t, pDependency,tDependency, SizeRecord, LengthRecord, SizeLengthRecord, Ratios, pCDK, Threshold_D, Daughter1RGRvar, Daughter2RGRvar)
            for j = 1:Steps
                t = t + 1
                n = length(tissue);
                m = 0;
                ...
                for i = 1:n
                  if tissue(i).Phase < 2;
                     m = m + 1;
                     ActiveCells(m) = i;
                     tissue(i).Size = tissue(i).Size + ((tissue(i).Size * (-0.0002 * tissue(i).BirthSize + 0.0338) + tissue(i).RGRvar));  
                    ...
                    elseif tissue(i).Phase == 1
                        ...
                        tissue(i).Phase = tissue(i).Phase + 1;
                        tissue(length(tissue) + 1) = CellOneCDK_GrowthDependBirthSize(Daughter1RGRvar(i), pCDK, tissue(i).DaughterSize1, 0, Threshold_D, 0, 0, tissue(i).DaughterSize1, 0, 0); 
                        tissue(length(tissue) + 1) = CellOneCDK_GrowthDependBirthSize(Daughter2RGRvar(i), pCDK, tissue(i).DaughterSize2, 0, Threshold_D, 0, 0, tissue(i).DaughterSize2, 0, 0);
       ...
       csvwrite('C:\Users\Angharad\Desktop\SizeData.csv', SizeRecord)        % to save data on cell size for further analysis enter a valid path here
       csvwrite('C:\Users\Angharad\Desktop\CycleLengthData.csv', LengthRecord)      % to save data on cell cycle length for further analysis enter a valid path here
